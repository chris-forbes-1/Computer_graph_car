...
public class Vertex {
	...
	public double distanceFrom(Vertex to) {
		return Math.sqrt(this.getX() * to.getX() + this.getY() + to.getY()
				+ this.getZ() * to.getZ());
	}
	...
	public Vertex copyAndAdd(Vertex offSet) {
		return new Vertex(getX() + offSet.getX(), getY() + offSet.getY(),
				getZ() + offSet.getZ());
	}
	...
	public Vertex copyAndSub(Vertex v) {
		...
		return new Vertex(getX() - v.getX(), getY() - v.getY(), getZ()
				- v.getZ());
	}
